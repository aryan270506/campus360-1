...
// ─── Sub-components ───────────────────────────────────────────────────────────
const LabBadge = ({ color = C.teal }) => (
  <View style={[s.labBadge, { backgroundColor: color + '33', borderColor: color }]}>
    <Text style={[s.labBadgeText, { color }]}>LAB</Text>
  </View>
);

const ClassCard = ({ event, isActive, compact }) => (
...
// ─── Weekly Grid ──────────────────────────────────────────────────────────────
const WeeklyGrid = ({ colWidth, timeColWidth }) => (
  <View>
    ...
    <View style={s.timeRow}>
      ...
      {SCHEDULE.map((day) => {
        ...
        return (
          <View key={day.day} style={[s.cellBlock, { width: colWidth }]}>
            {evt && <ClassCard event={evt} isActive={day.day === 'TUE'} compact={colWidth < 100} />}
          </View>
        );
      })}
    </View>
    ...
    <View style={s.timeRow}>
      ...
      {SCHEDULE.map((day) => {
        ...
        return (
          <View key={day.day} style={[s.cellBlock, { width: colWidth }]}>
            {evt && <ClassCard event={evt} compact={colWidth < 100} />}
          </View>
        );
      })}
    </View>
    ...
    {SCHEDULE.map((day) => (
      <View key={day.day}>
        ...
        <View style={{ gap: 8, marginTop: 8 }}>
          {day.events.length > 0
            ? day.events.map((evt) => <ClassCard key={evt.id} event={evt} />)
...
export default function ParentSchedule({ onMenuOpen }) {
  ...
  // ✅ KEY FIX: measure actual rendered width via onLayout — not useWindowDimensions
  // This gives us the TRUE available width after the sidebar has taken its space
  ...
  <View style={s.searchBar}></View>
        <View style={s.searchBar}>
          <Text style={{ color: C.muted, fontSize: 13 }}>🔍  Search courses, instructors...</Text>
        </View>
        <View style={{ flexDirection: 'row', gap: 8 }}>
          ...
          {containerWidth > 500 && (
            <View style={s.profilePill}>
              <View>
                <Text style={{ color: C.white, fontSize: 13, fontWeight: '600', textAlign: 'right' }}>Alex Johnson</Text>
                <Text style={{ color: C.sub, fontSize: 11, textAlign: 'right' }}>Computer Science</Text>
              </View>
              <View style={s.avatar}>
                <Text style={{ color: C.blueLight, fontSize: 13, fontWeight: '700' }}>AJ</Text>
              </View>
            </View>
          )}
        </View>
      ...
      {/* Schedule content — width is now correctly bounded to what onLayout measured */}
      <View style={s.scheduleBody}>
        {viewMode === 'Weekly' ? (
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <WeeklyGrid colWidth={colWidth} timeColWidth={timeColWidth} />
            </ScrollView>
          </ScrollView>
        ) : (
          <DailyList />
...
const s = StyleSheet.create({
  // ✅ flex:1, no fixed width, no overflow issues — fits exactly inside shell
  ...
  timeRow: {
    ...
    borderBottomWidth: 1, borderBottomColor: '#0d1829',
  },
  ...
  labBadge: { paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4, borderWidth: 1 },
  labBadgeText: { fontSize: 8, fontWeight: '800', letterSpacing: 0.5 },
